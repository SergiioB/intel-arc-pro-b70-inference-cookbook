...
| Qwen3.8-27B GPTQ-INT4, single B70 | `…f01e24f6c7ff…` | Qwen MTP: `patch_mtp_nightly.py` then `patch_mtp_boundary.py`. Optional mixed-batch: compact+scatter `patch_gdn_mixed_split_v5.py`. Optional MTP speed: `patch_draft_lmhead_int4.py` then `patch_draft_mtp_int4.py` (`B70_DRAFT_LMHEAD_INT4=1`, `B70_DRAFT_MTP_INT4=1`). Optional correctness when `--enable-prefix-caching` is ON: `patch_fix_accepted_sync.py`, `patch_fix_backward_copy.py`, `patch_fix_eagle_drop.py` (hybrid MTP + align; dry-run without GPU via `scripts/verify-mtp-apc-fixes.sh`; context in [RELIABILITY-REPORT](RELIABILITY-REPORT.md) §1). [Recipe](qwen38-27/QWEN38-VLLM-XPU.md). | Nemotron grouped-topk / SSU. FP8 W8A16/small-M patches. Original full-buffer GDN split (OOB on kernels 0.1.12.3). DFlash 2 / PR #52816 is **not** an apply-list item: overlay can load, measured accept **0%** |
| Qwen3.8-27B FP8 W8A16, dual B70 TP2 | `…f01e24f6c7ff…` + rebuilt matching `vllm-xpu-kernels 0.1.12.3` | Source build: `vllm-xpu-kernels/apply_smallm_patch.py`, then `fix_smallm_placement.py`. Runtime: `patch_fp8_stack.py`; add `patch_fp8_hybrid.py` for hybrid W8A8-prefill/W8A16-decode mode; add `patch_vllm_worker_affinity.py` plus the dual-B70 oneCCL/SYS_PTRACE contract. [Recipe](qwen38-27/FP8-TP2-W8A16.md). | GPTQ draft-INT4 overlays, Qwen BF16-draft MTP patch pair as a substitute for the FP8 model's native MTP weights, Nemotron grouped-topk / SSU. Do not use the FP8 values as GPTQ benchmarks. |
...
| Qwen3.8-Flash-Next (llama.cpp SYCL, two B70s, C1) | llama.cpp `9723942adc518b43c4b95dc4dce6906903eb5e09` (not a vLLM image) | `patches/llamacpp-sycl/flashnext-arch-overlay.patch`, then `sycl-fused-mmvq-and-gpu-group.patch`. Pick one `GGML_SYCL_F16` binary. [Recipe](qwen38-flash-next/QWEN38-FLASH-NEXT-LLAMACPP.md). | All vLLM Qwen MTP / FP8 / Nemotron / Ornith patches. Do not apply the older split SYCL files on top of the combined kernel patch. 8K/16K/128K are context, not concurrency. |
...
draft-INT4 S+M1 patches, with prefix cache **on** for serving. Serving APC-on + MTP on this family also wants the three correctness fixes below (§1 of the reliability report). `DRAFT_INT4=0`
